...
def run_experiment(root_dir, seed, queue_depth):
    i = queue_depth
    result = experiment2(
        epochs=queue_depth * 2_000_000, queue_depth=queue_depth, seed=seed
    )
    fig = plot_single_run_metrics(result)
    fig.savefig(f"{root_dir}/plot_{i}.png")
    plt.close(fig)
    with open(f"{root_dir}/logs_{i}.log", "w") as f:
        str_logs = ""
        for i in tqdm(range(len(result["logs"]))):
            str_logs += str(result["logs"][i]) + "\n"
        f.write(str_logs)


def main():
    k = 5
    root_dir = f"results/queue_depth_change_epochs_change_seed_{k}"
    os.makedirs(root_dir, exist_ok=True)
    with ProcessPoolExecutor(max_workers=64) as executor:
        futures = {
            executor.submit(run_experiment, root_dir, k, queue_depth)
            for queue_depth in range(10, 200, 10)
        }
        for future in tqdm(
            as_completed(futures), total=20, desc="Running Experiments..."
        ):
            future.result()
